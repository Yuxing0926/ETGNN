...
+ `setup`:
    ...
    + `property`: Select the type of physical quantity to be output by the network: scalar ('scalar'), atomic average scalar ('scalar_per_atom'), Born effective charge ('Born'), force ('force'), dielectric ('dielectric').
    + `l_pred_atomwise_tensor`: Set to True to predict tensors for each atom.
    + `l_pred_crystal_tensor`: Set to True to predict the tensor properties of the crystal.
...
+ `losses_metrics`:
    + `losses`: define multiple loss functions and their respective weights in the total loss value. Currently, ETGNN supports mse, mae, cosine_similarity, sum_zero and Euclidean_loss.
...
+ `optim_params`:
    ...
    + `lr`: learning rate, the default value is 0.0005.
+ `profiler_params`:
